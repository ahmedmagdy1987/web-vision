...
function SheetContent({
  ...
  showClose = true,
  ...props
}: React.ComponentProps<typeof SheetPrimitive.Content> & { side?: Side; showClose?: boolean }) {
  const sideClasses: Record<Side, string> = {
    ...
    bottom:
      "inset-x-0 bottom-0 h-auto rounded-t-xl border-t data-[state=open]:slide-in-from-bottom data-[state=closed]:slide-out-to-bottom",
  };
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "bg-background fixed z-50 flex flex-col gap-4 p-6 shadow-lg transition ease-in-out",
      ...
      >
        {children}
        {showClose && (
          <SheetPrimitive.Close className="absolute top-4 right-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus-visible:ring-2 focus-visible:ring-ring/50 outline-none">
            <X className="size-4" />
